...
## 26. Key Reference Summary

### Alert Triage Essentials

- Process of validating alerts is called **alert triage**
- **False Negative** is the most dangerous alert type
- **Tune rules** to reduce alert fatigue
- **MTTD / MTTR** = Detect time / Response time
- SOC report includes: **Incidents + threats + metrics**

### Core SIEM Questions

- Logs used in SIEM: **Firewall, IDS, server, application**
- **Correlation** = Linking multiple events to detect attack
- **UEBA vs SIEM:** UEBA = behavior analytics, SIEM = log management + detection
- **False Positive** = Alert triggered but no real threat

### Data Exfiltration Essentials

- Monitor outbound traffic
- Detect anomalies in volume
- Use DLP + SIEM correlation

### Insider Threat Essentials

- Authorized user misusing access
- Long DNS queries + high frequency + encoded domains = DNS exfiltration
- **HTTPS exfiltration** is hard because encryption hides payload

### Insider Threat Detection Logs
...
> **End of CSA Module 4 – SIEM & SOC Detection Mastery**
